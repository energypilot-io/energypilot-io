...
import * as buffers from 'h5.buffers'
import errors from '@csllc/cs-modbus/lib/errors'
import Transport from '@csllc/cs-modbus/lib/Transport'
...
AsciiTransport.prototype.frame = function (unit, pdu) {
    var frame = Buffer.alloc(pdu.length + 2)
    var i = 0
...
AsciiTransport.prototype.decodeByte = function (highNibble, lowNibble) {
    return (
        (this.decodeNibble(highNibble) << 4) +
...
AsciiTransport.prototype.isValidChunk = function (chunk) {
    return this.reader.length > 0 || chunk.toString()[0] === '>'
}
 ...
 * @returns {Error|null}
 */
AsciiTransport.prototype.validate = function (
    transaction,
    frame,
    expectedChecksum
) {
    ...
    if (actualUnit !== expectedUnit) {
        return new errors.InvalidResponseDataError(
            util.format(
